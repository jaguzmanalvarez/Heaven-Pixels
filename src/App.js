...
const App = () => {
  ...
  const handleSaveEditedReview = () => {
    if (editingReview) {
      // Actualiza la reseña en el estado
      setGames((prevGames) =>
        prevGames.map((game) => ({
          ...game,
          reviews: game.reviews.map((review) =>
            review.id === editingReview.id
              ? { ...review, rating: newRating, comment: newComment } // Modifica la reseña
              : review
          ),
        }))
      );
    ...
    return (
      <div className="App">
        ...
        <ReviewList
          ...
          handleEditReview={handleSaveEditedReview}
